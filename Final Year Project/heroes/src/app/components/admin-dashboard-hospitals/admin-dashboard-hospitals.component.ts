...
@Component({
  selector: 'app-admin-dashboard-hospitals',
  templateUrl: './admin-dashboard-hospitals.component.html',
  styleUrls: ['./admin-dashboard-hospitals.component.css']
})
export class AdminDashboardHospitalsComponent implements OnInit {
  ...
  blockDoctor(id: any) {
    ...
    this.api.getHospital().subscribe(
      res => {
        for (var j in res) {
          if (res[j].id == id) {
            this.temp = { "id": res[j].id, "city_id": { "name": res[j].city_id.name }, "name": res[j].name, "location": res[j].location, "contact": res[j].contact, "block_status": 1 }
          }
        }
